...
template <typename T>
void OpMatrices<T>::imprimirMatriz(Matriz<T> matrizGeneral) {
	for (int i = 0; i < matrizGeneral.getDim(); i++) {
		for (int j = 0; j < matrizGeneral.getDim(); j++) {
			std::cout << *( *(matrizGeneral.getPuntMatriz() + i) + j) << " ";
		}
		std::cout << "\n";
	}
}
